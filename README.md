...
**RecoveryAI** (Agent Recovery Protocol - ARP) is a lightweight, stateless diagnostic microservice designed specifically to resolve execution failures in multi-agent systems. When autonomous AI agents perform tasks, they frequently encounter failed HTTP requests or out-of-order workflow actions. Instead of returning raw, developer-facing HTTP errors (e.g. `405 Method Not Allowed`), RecoveryAI:
...
> **Design Philosophy**: To maintain strict execution simplicity for LLMs and autonomous agents, RecoveryAI handles diagnostics and retry plans **server-side**. This removes the need for client agents to parse raw logs, guess headers, or bundle complex error-handling libraries locally.
...
- **Deterministic Error Taxonomy:** The system restricts LLM responses to a strict, pre-defined taxonomy of 13 failure states (e.g. `AUTH_ERROR`, `METHOD_ERROR`, `PARAMETER_ERROR`, `RATE_LIMIT`), eliminating model hallucinations.
...
```text
recovery_ai/
├── main.py             # FastAPI router, validation schemas, and endpoints
...
### 3. Explain Error
`POST /explain`
Explains HTTP error status and body response details in simple terms for autonomous agents.
- **Request:**
  ```json
  {
    "status": 429,
